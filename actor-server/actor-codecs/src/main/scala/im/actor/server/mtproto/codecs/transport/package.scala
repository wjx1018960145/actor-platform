...
import im.actor.server.mtproto.codecs.primitive.{IntLengthBytesCodec, IntLengthStringCodec}
...
package object transport {
  ...
  private val intLengthBits = IntLengthBytesCodec

  private val intLengthString = IntLengthStringCodec
  ...
  val PackageCodec = (C.int64 :: C.int64 :: C.bits).as[MTPackage]
  ...
  def mtprotoCodec(header: Int): GenCodec[_, MTProto] =
    header match {
      ...
      case MTPackage.header ⇒ PackageCodec.map(_.asInstanceOf[MTProto])
